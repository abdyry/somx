...
class sTAFFDataTable extends DataTable
{
    public function ajax(): JsonResponse
    {
        return datatables()
            ...
            ->addColumn('action', function ($user) {
                ...
                $delete = (Common::has_permission(auth('admin')->user()->id, 'delete_staff')) ? '<a href="' . url(config('adminPrefix') . '/staff/delete/' . $user->id) . '" class="btn btn-xs btn-danger delete-warning"><i class="fa fa-trash"></i></a>' : '';
                return $edit . $delete;
            })
    ...
    {
        $query = User::with(['document_verification:id,user_id,status', 'role:id,display_name', 'user_detail:id,user_id,last_login_at,last_login_ip'])
        ->where('role_id', 5)
        ->select('users.*');
